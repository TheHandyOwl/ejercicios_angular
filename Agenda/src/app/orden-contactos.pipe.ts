...
// Un pipe es una clase decorada con 'Pipe'. Es necesario que indiquemos el metadato 'name', ya que es el identificador del pipe que usaremos en los templates para usarlo.
// Un pipe tiene que implementar siempre la interfaz 'PipeTransform', que a su vez nos obliga a implementar la función 'transform'.
@Pipe({
  name: 'OrdenContactos'
})
  ...
  transform(contactos: Contacto[]): Contacto[] {
    ...
    if (contactos){
      ...
      ordenados = contactos.sort((contactoA: Contacto, contactoB: Contacto): number => {
        ...
        // Si el nombre del contacto B es mayor que el nombre del contacto A, significa que la ordenación A-Z es correcta y, por tanto, como resultado de la comparación retornamos -1
        else if(contactoA.nombre < contactoB.nombre) {
          resultado = -1;
        }

        // Si el nombre del contacto A es igual que el nombre del contacto B, significa que la ordenación A-Z es correcta porque son iguales y, por tanto, como resultado de la comparación retornamos 0
        else {
          resultado = 0;
        }
